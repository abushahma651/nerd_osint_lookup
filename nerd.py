...
def banner_fade_in():
    clear()
    for style, d in [(DIM_G, 220), (CYAN, 260), (WHITE, 300)]:
        clear()
        _c(BANNER, style)
        _slp(d)
...
def welcome_typing():
    text = "Welcome Nerd"
    palette = [CYAN, CYAN, "cyan", "cyan", WHITE, WHITE, "cyan", "cyan", MAG, MAG, CYAN, CYAN]
    t = Text()
...
def elegant_bar(text="Initializing", duration=1.6, width=26):
    steps = int(duration / 0.035)
    for i in range(steps + 1):
        pct = i / steps
        filled = int(width * pct)
        bar = "▓" * filled + "░" * (width - filled)
        line = f"[{GREY}]{text}[/{GREY}]  [{CYAN}]{bar}[/{CYAN}]  [{WHITE}]{int(pct*100):3d}%[/{WHITE}]"
...
def soft_spinner(text="Connecting", duration=1.0):
    frames = ["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"]
    ...
    while time.time() < end:
        console.print(
            Align.center(Text.from_markup(
                f"[{CYAN}]{frames[i % len(frames)]}[/{CYAN}]  [{GREY}]{text}[/{GREY}]"
            )),
            end="\r"
        )
        sys.stdout.flush()
...
def welcome_animation():
    ...
    _c("◆   O S I N T   L O O K U P   ◆", CYAN)
    _slp(250)
    _c(f"by {DEVELOPER}", GREY)
    _slp(250)
    console.print()
    ...
    elegant_bar("Starting engine", duration=1.5)
    soft_spinner("Initializing modules", duration=1.0)

    _slp(300)
    clear()

def warn_panel_animated():
    for shade in [DIM_G, GREY, "grey70", "red"]:
        clear()
        _c(BANNER, CYAN)
        console.print()
        body = Text()
        ...
        body.append("Use at your own risk.", style=GREY)
        console.print(Panel(body, border_style=shade, box=ROUNDED))
        _slp(120)

def dev_panel_animated():
    for shade in [DIM_G, GREY, CYAN]:
        body = Text()
...
def about_dev():
    ...
    console.print(Panel(f"[bold white]{BRAND} research tool by {DEVELOPER}.[/bold white]\n\n"
                        f"[grey50]Check if YOUR number leaked.[/grey50]\n\n"
                        f"[bold magenta]Telegram:[/bold magenta]\n  • {CH_TELEGRAM_1}\n\n"
                        f"[bold cyan]Do not remove credits.[/bold cyan]",
                        border_style="cyan", title="[bold cyan]ABOUT[/bold cyan]", box=ROUNDED))
    console.print()
    Prompt.ask("[grey50]press ENTER[/grey50]", default="", show_default=False)
...
def main():
    ...
    dev_panel_animated()
    console.print()
    console.print(Align.center(Text.from_markup("[bold cyan]press ENTER to start[/bold cyan]")))
    input()
